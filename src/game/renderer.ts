...
export class Renderer {
  ...
  drawAmbulance(x: number, y: number, now: number): void {
    ...
    this.ctx.quadraticCurveTo(x - flameWidth * 0.8, y + 64 + flameHeight * 0.6, x, y + 64 + flameHeight);
    this.ctx.quadraticCurveTo(x + flameWidth * 0.8, y + 64 + flameHeight * 0.6, x + flameWidth / 2, y + 64);
    this.ctx.fill();
  ...
  drawCapsule(capsule: CapsuleEntity): void {
    ...
    const trailGrad = this.ctx.createLinearGradient(capsule.x, capsule.y + 26, capsule.x, capsule.y + 40);
