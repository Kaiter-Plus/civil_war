...
const game = {
  ...
  knowledge: $.ajax({
    type: 'GET',
    url: './res/strings/knowledge.json',
    dataType: 'json',
    success: function (data) {
      game.knowledge = data
    },
  ...
  // 游戏结算界面提示信息
  tipHTML: function (tip, flag, score) {
    switch (flag) {
      case 0:
        tip.html(
          [
  ...
  // 开始界面按钮点击事件
  buttonClick: function (index, showMessage) {
    switch (index) {
      // 开始游戏
      case 0:
        // 游戏开始标志
        ...
        // 播放背景音乐
        game.timer.backgroundMusic = setTimeout(function () {
          // 开始播放背景音乐
        ...
        game.timer.startGame = setTimeout(function () {
          // 调用函数绘制救护车以及分数栏
          game.core.draw()

          // 胶囊发射定时器
          game.timer.capsule = setInterval(function () {
            game.core.capsule(set[0], [game.num.ambulanceX, game.num.ambulanceY])
          }, set[1])

          // 病毒出现定时器
          game.timer.virus = setInterval(function () {
            game.core.virus({
  ...
  startScreen: {
    ...
    draw: function () {
      ...
      const buttonGroup = $('<div>')
        .addClass('buttonGroup col-8')
        .html(
          [
            "<a class='btn btn-block btn-lg btn-success' href='javascript:;'>开始游戏</a>",
            "<a class='btn btn-block btn-lg btn-primary' href='javascript:;' data-toggle='modal' data-target='#modal'>查看规则</a>",
            "<a class='btn btn-block btn-lg btn-warning' href='javascript:;' data-toggle='modal' data-target='#modal'>防疫小知识</a>",
            "<a class='btn btn-block btn-lg btn-danger' href='javascript:;' data-toggle='modal' data-target='#modal'>谣言我先知</a>",
          ].join('\n')
        )
      ...
      buttonGroup.on('click', 'a', function () {
        ...
        game.buttonClick($(this).index(), showMessage)
      })
    },
  ...
  core: {
    ...
    draw: function () {
      ...
      const tipButton = $("<button id='tipButton' style='display:none' data-toggle='modal' data-target='#modal'>")
      game.stage.append(tipButton)

      // 记录胶囊发射初始位置
      game.num.ambulanceX = parseInt(ambulance.css('left')) + ambulance.width() / 2
      game.num.ambulanceY = parseInt(ambulance.css('top'))
    },

    // 救护车位置
    ambulance: function (pos) {
      let ambulance = game.stage.find($('.ambulance')),
        left = pos[0] - ambulance.width() / 2,
        top = pos[1] - ambulance.height() / 2
    ...
    // 病毒
    virus: function (argument) {
      const speed = argument.speed
      const left = argument.left
      const top = argument.top
      const $virus = $('<div>')
      $virus
        .addClass('virus')
        .css({
      ...
      $virus.timer = setInterval(function () {
        // 获取救护车对象
        const ambulance = $('.ambulance')
        // 获取屏幕上的所有胶囊
        let capsule = $('.capsule')
        // 胶囊击中，病毒消失
        let x = parseInt($virus.css('left')) + 32,
          y = parseInt($virus.css('top')) + 32,
          l = capsule.length

        // 病毒击中消失
        for (let i = 0; i < l; i++) {
          let dx = Math.abs(x - parseInt(capsule.eq(i).css('left')) - 8),
            dy = Math.abs(y - parseInt(capsule.eq(i).css('top')) - 13)
          if (dx <= 40 && dy <= 45) {
            // 病毒击中效果
            $virus.css('background', "url('res/img/killed.png')")
            // 击毁病毒音效
        ...
        // 救护车损坏，显示分数以及成就
        let dx2 = Math.abs(x - parseInt(ambulance.css('left')) - 15),
          dy2 = Math.abs(y - parseInt(ambulance.css('top')) - 32)
        ...
        if (dx2 <= 47 && dy2 <= 64 && !$('.tips').get(0)) {
          ...
          // 结算时的防疫小知识
          game.stage.delegate('#tipButton', 'click', function () {
          ...
          $virus.remove()
          score.addClass('none')
          ambulance.css('background', "url('res/img/isDestroy.png')")
          clearInterval($virus.timer)
          setTimeout(function () {
            $('.ambulance').remove()
          }, 400)
  ...
  randomNum: function (a, b) {
    let value = Math.abs(a - b),
      num
    num = Math.floor(Math.random() * value) + Math.min(a, b)
    return num
  },
}
