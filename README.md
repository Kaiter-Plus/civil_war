...
- **Lint**: oxlint（Rust 编写，极速检查）
- **Format**: oxfmt（Prettier 风格，无分号、单引号、2空格）
- **Hooks**: simple-git-hooks（pre-commit 自动 lint + format）
...
- 新增：6 种病毒移动模式（直线、正弦波、Z字形、追踪、螺旋、俯冲）
- 新增：oxlint + oxfmt 工具链 + pre-commit hooks
- 配置：Prettier 风格格式化（无分号、单引号、2空格缩进）
- 优化：代码格式化统一
